...
import { youtubeNotificationHandler } from '../handlers/youtubeNotify';
...
export const NotificationStatus: React.FC = () => {
  ...
  useEffect(() => {
    // 初期状態を取得
    setIsActive(youtubeNotificationHandler.isActive());

    // 定期的に状態を更新
    const interval = setInterval(() => {
      setIsActive(youtubeNotificationHandler.isActive());
      if (youtubeNotificationHandler.isActive()) {
  ...
  const handleToggle = () => {
    if (isActive) {
      youtubeNotificationHandler.stop();
      setIsActive(false);
    } else {
      youtubeNotificationHandler.start();
      setIsActive(true);
      setLastCheck(new Date().toLocaleTimeString('ja-JP'));
    }
  };
    ...
    <div className="bg-[#36393f] rounded-lg p-4 border border-[#2f3136]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-medium">通知システム</h3>
      ...
      {!apiStatus.youtube && (
        <div className="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded flex items-center gap-2">
          ...
          <span className="text-yellow-400 text-sm">
            YouTube API キーが設定されていません。環境変数 VITE_YOUTUBE_API_KEY を設定してください。
          </span>
        </div>
      )}
    </div>
